var FileStorageStore = function() {
    ...
    this.flush = function(type, data) {
        ...
        self.bufferSize = self.memBuffer[type].length;
        ...
        if (self.bufferSize >= self.cachedSamples) {
            self.bufferSize = 0;
            self.memBuffer[type].length = 0;
            
            var fileName = type + '.data.log';
            var self_type = type;
            var toTransfer = ((self.countSamples[type] % this.maxSampleChunk)) <= 10 ? true : false;
            Utils.showPopup(self.countSamples[type] + " " + toTransfer);
    ...
    var appendToFileJSON = function(type, toTransfer, fileEntry) {
        ...
        fileEntry.createWriter(function(fileWriter) {
                               ...
                               self.transfer().done(function() {
                                                    self.clearAll();
                                                    Utils.showPopup("Data Transferred");
                                                    })
                                               .fail(Utils.showPopup("Network Error. Data not transferred"));
                               }
                            }, errorHandler);
    };
