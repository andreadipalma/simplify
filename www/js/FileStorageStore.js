var FileStorageStore = function() {
    ...
    this.maxSampleChunk = 200;
    this.gap = 0;
    this.fileURI = null;
    this.printCountCallBack = null;
    this.callback = null;
    this.type = null;
    ...
    this.setPrintCountCallBack = function(method) {
        this.printCountCallBack = method;
    };
    ...
    this.clearAll = function() {
        async(function() {
                  // Clear local storage
                    ...
                    FileManager.delete("acceleration.data.log");
                    FileManager.delete("orientation.data.log");
                    FileManager.delete("position.data.log");
              
                    self.countSamples.acceleration = 0;
                    self.countSamples.orientation = 0;
                    self.countSamples.position = 0;
                  });
    };
    ...
    this.flush = function(type, data) {
        ...
        var transferFile = function(toTransfer) {
                if (toTransfer === true) {
                    self.gap = self.countSamples[type] % self.maxSampleChunk;
                    Utils.showPopup("gap="+self.gap);

                    //self.compress().then(self.storageManager.transfer).done(self.clearAll());
                    self.transfer().done(function() {
                                         Utils.showPopup("Data Transfered");
                                         FileManager.delete("acceleration.data.log");
                                         })
                    .fail(function() {Utils.showPopup("Network Error. Data not transferred");});
                }
        };
        ...
        self.countSamples[type]++;
        
        if (self.printCountCallBack !== null) {
            self.printCountCallBack(self.countSamples[type]);
        }
        ...
        if (self.memBuffer[type].length === self.cachedSamples) {
            ...
            var fileName = type + '.data.log';
            var self_type = type;
            var toTransfer = ((self.countSamples[type] % this.maxSampleChunk)) <= self.gap+5 ? true : false;
            
            // Check whether the file exist and create a new file or append to the existing one
            FileManager.checkExist(fileName)
                            .done(function(handler) {
                                    if ((self.fileURI === 'undefined') || (self.fileURI === null)) {
                                        self.fileURI = encodeURI(handler.nativeURL);
                                        //Utils.showAlert('fileURI = ' + fileEntry.nativeURL + ' - ' + JSON.stringify(fileEntry));
                                    }
                                  
                                    FileManager.write(false, fileName, self.memBuffer[type])
                                        .done(function () {
                                              console.log('Done Append going to transfer.');
                            ...
                            .fail(function(handler) {
                                    ...
                                    FileManager.write(true, fileName, self.memBuffer[type])
                                        .done(function () {
                                              console.log('Done Create going to transfer.');
                                              self.memBuffer[type].length = 0;
                                              transferFile(toTransfer);
                                            })
                                        .fail();
